...
for file_name in os.listdir(test_path):
    if not file_name.startswith(".") and os.path.isfile(os.path.join(test_path, file_name)):
...
parser.add_argument("--thresh",help ="Value for probability threshold",default=0.5,type=int)
...
# Extract file names
file_names = []
for path in pred_file_names:
    # Use the basename() method from the os.path module to extract the file name
    new_name = os.path.basename(path).replace('.nii', '')
...
for idx, i in enumerate(pred_cls):
    ...
    if (i==0):
        if fname != './input/test/bad/.DS_Store':
            copy(fname, DESTN_FP + 'bad/' + img_name)
            results.append([file_names[idx], 'bad', gt_cls_name[idx]])
    else:
        if fname != './input/test/good/.DS_Store':
            results.append([file_names[idx], 'good', gt_cls_name[idx]])
            copy(fname, DESTN_FP + 'good/' + img_name)

# create a DataFrame from results excluding the specified file paths
df = pd.DataFrame(results, columns=['File Name', 'Predicted Class', 'GT Class'])
df = df[~df['File Name'].isin(['./input/test/bad/.DS_Store', './input/test/good/.DS_Store'])]

# write the filtered DataFrame to a CSV file
df.to_csv('./raw_output.csv', index=False)